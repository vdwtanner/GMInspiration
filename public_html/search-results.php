<?php
	...
	$pageTitle = "Search - The GM's Inspiration";
	$pageDescription = "Search The GM's Inspiration for some homebrew content that will liven up your next session";
	include "header.php";
	session_start();
?>
<DOCTYPE html>
...
*/
	...
	echo "<form method=GET class='inlineform' action='search-results.php'>";
	...
	if(($offset - $resultLimit) >= 0){
		...
		echo "<a class='button prev' href='search-results.php?keywords=".$ekeywords."&csort=".$csort."&offset=".($offset - $resultLimit)."'>&nbspprev</a>";
	}	
	if(($offset + $resultLimit) < $resulttotal){

		$csort = htmlspecialchars($_GET["csort"], ENT_QUOTES, "UTF-8");
		echo "<a class='button next' href='search-results.php?keywords=".$ekeywords."&csort=".$csort."&offset=".($offset + $resultLimit)."'>next&nbsp</a>";
	}
	...
	if($crowarr){
		...
		foreach($crowarr as $key => $value){
				...
				echo "<a href='view-contribution-updateable.php?contid=".$value["id"]."' style='inline-block'>";
				if($value["privacy"] == 1){
				...
				echo "</div>";
				echo "<div class='searchtext'>";
				echo "<a href='view-contribution-updateable.php?contid=".$value["id"]."'>";
				if($value["sub_type"])
